...
%%%%%%%%Transpose the input signal to row to meet habit
L = min(size(se,1),size(s,1));
se = se(1:L,:)';
s = s(1:L,:)';
...
for jest=1:nsrc
    for jtrue=1:nsrc
        [s_true,e_spat,e_interf,e_artif]=bss_decomp_mtifilt(se(jest,:),s,jtrue,64);%%%the length of filter,origin 512, adjusting to 64
